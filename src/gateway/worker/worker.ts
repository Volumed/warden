...
let gatewayQueue: Queue | undefined

if (workerData.messageQueue.enabled) {
  gatewayQueue = new Queue('gatewayMessage', {
    connection: {
      host: workerData.messageQueue.host,
      port: workerData.messageQueue.port,
    },
  })
}

const handleShardMessageEvent = async (shard: DiscordenoShard, payload: Camelize<DiscordGatewayPayload>) => {
  const body = JSON.stringify({ payload, shardId: shard.id })

  if (workerData.messageQueue.enabled) {
    if (!gatewayQueue) {
      logger.error('The BullMQ queue has not been created. The event will be lost')
      return
    }

    const deduplicationHash = createHash('sha1')
    deduplicationHash.update(JSON.stringify(payload.d))
    const deduplicationId = deduplicationHash.digest('hex')

    await gatewayQueue
      .add('event', { payload, shardId: shard.id }, { deduplication: { id: deduplicationId } })
      .catch((error) => logger.error('Failed to add event to BullMQ queue', error))
...
const createShard = (shardId: number): DiscordenoShard => {
...
parentPort.on('message', async (message: WorkerMessage) => {
  ...
  if (message.type === 'SwitchShards') {
    ...
    for (const shard of shards.values()) {
      ...
      shard.events.message = async (_, message) => {
